...
export function PrivacyDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Privacy choices">
      ...
      <div className="mt-5 flex items-center justify-between gap-4 rounded-lg bg-surface-container-high p-4">
        <div className="min-w-0">
          <p className="font-medium">Product analytics</p>
          <p className="mt-0.5 text-body-medium text-on-surface-variant text-pretty">
        ...
        <button
          ...
          className={cn(
            ...
            privacy.analytics ? 'bg-primary' : 'bg-outline',
          )}
        >
          <span
            className={cn(
              'absolute top-1 size-5 rounded-full bg-surface-container shadow-level2 transition-[left] duration-[var(--duration-base)] ease-[var(--ease-out)]',
      ...
      <section className="mt-5">
        <h4 className="text-body-medium font-medium text-on-surface">What we collect</h4>
        <ul className="mt-2 flex flex-col gap-1.5">
          {WHAT_IS_COLLECTED.map((line) => (
            <li key={line} className="flex gap-2 text-body-medium text-on-surface-variant">
              <Check size={14} className="mt-[3px] shrink-0 text-primary" aria-hidden />
              <span className="text-pretty">{line}</span>
            </li>
          ))}
        </ul>
        <h4 className="mt-4 text-body-medium font-medium text-on-surface">What we never collect</h4>
        <ul className="mt-2 flex flex-col gap-1.5">
          {WHAT_IS_NEVER_COLLECTED.map((line) => (
            <li key={line} className="text-body-medium text-on-surface-variant text-pretty">
              {line}
            </li>
          ))}
        </ul>
