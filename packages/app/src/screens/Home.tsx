...
const QUICK = ['How Transformers work in LLMs', 'Swift fundamentals', 'Reading an ECG strip', 'How TCP handshakes work'];
...
export function Home() {
  ...
  const categories = useMemo(() => ['All', ...new Set((sessions ?? []).map((s) => DOMAIN_LABELS[s.domain] ?? 'Other'))], [sessions]);
  const visible = useMemo(() => {
    const f = filter.trim().toLowerCase();
    return (sessions ?? []).filter((s) => (category === 'All' || (DOMAIN_LABELS[s.domain] ?? 'Other') === category) && (!f || `${s.title} ${s.topic} ${experts.get(s.expertId)?.displayName ?? ''}`.toLowerCase().includes(f)));
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <section className="px-7 pt-[76px] pb-[60px]">
        <div className="mx-auto flex w-full max-w-[756px] flex-col items-center">
          <h1 className="mb-8 text-center text-3xl leading-[1.02] tracking-[-0.036em] text-fg text-pretty">What do you want to learn?</h1>
          <form
          ...
          >
            ...
            <input
              ...
              autoFocus
            />
            <button type="button" title="Say it instead" aria-label="Say it instead" className="grid size-11 shrink-0 place-items-center rounded-[11px] text-fg-3 transition-colors hover:bg-surface-2 hover:text-fg" onClick={() => toast('Voice search arrives with the room microphone: press Start and just talk.')}>
              <Mic size={19} />
            </button>
            <span className="h-7 w-px shrink-0 bg-line-strong" aria-hidden />
            <Button variant="primary" size="lg" type="submit" disabled={!query.trim()} loading={starting} trailing={<ArrowRight size={16} />}>
              Start session
            </Button>
          </form>
      ...
      <div className="sticky top-[65px] z-[9] bg-bg px-7 pt-[22px] pb-2.5">
        <div className="mx-auto flex max-w-[1360px] items-center gap-3">
          ...
          <label className="flex h-7 w-[280px] shrink-0 items-center gap-2 rounded-[var(--radius-sm)] bg-surface px-2.5 hairline">
            ...
            <input className="min-w-0 flex-1 bg-transparent text-xs text-fg outline-none placeholder:text-fg-3" placeholder="Filter sessions" value={filter} onChange={(e) => setFilter(e.target.value)} aria-label="Filter sessions" />
          </label>
        </div>
      </div>
        ...
        <div className="mx-auto grid max-w-[1360px] grid-cols-[repeat(auto-fill,minmax(288px,1fr))] gap-x-[18px] gap-y-[34px]">
          {sessions === null
            ...
            : visible.map((s) => {
                ...
                return <SessionCard key={s.id} session={s} expertName={expert?.displayName ?? 'AI expert'} portraitUrl={api.portraitUrl(expert?.portrait?.src)} onOpen={() => navigate(`/sessions/${s.id}`)} />;
              })}
          {sessions !== null && visible.length === 0 ? (
            <div className="col-span-full flex flex-col items-center gap-2 py-16 text-center">
              ...
              <p className="text-sm text-fg-2">Start a session above — it becomes the first one in this list.</p>
            </div>
          ) : null}
        </div>
      </main>
