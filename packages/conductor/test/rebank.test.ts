...
function setup() {
  ...
  // Joining empties the player (its clock predates the room's stream). That
  // cancel is not a re-take, so the counters start from the joined room.
  audio.cancelled = 0;
  audio.enqueued.length = 0;
