import type { Expert, LessonPlan, SelectionBand, SessionMeta, TelemetryPort } from '@pen/contracts';
import {
  ModelSessionMeta,
  normaliseSessionMeta,
  SessionMeta as SessionMetaSchema,
} from '@pen/contracts';
import { type LanguageModel, type Usage, withTelemetry } from '@pen/llm';
...
export interface SessionMetaInput {
  ...
  cacheKey: string;
  /**
   * The session's telemetry port (ADR-0011): each attempt lands as one `llm`
   * stage sample and its `CostLine`s under purpose `session_meta`, so the
   * Insights tab and the cost totals include the card copy and sketch.
   */
...
export class SessionMetaJobs {
  ...
  private async run(input: SessionMetaInput): Promise<void> {
    ...
    let attempts = 0;
    // The model is shared by every session; the port is not, so the wrap is per job.
    const model = input.telemetry ? withTelemetry(this.o.model, input.telemetry) : this.o.model;
