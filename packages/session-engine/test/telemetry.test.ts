...
import { gapMsFor, PACE_DEFAULT } from '@pen/contracts';
...
describe('session telemetry (engine)', () => {
  it('records resolve/context/llm/tts/turn/join stages, cost lines, reports and errors in the ledger', async () => {
    ...
    // The beat after a sentence (ADR-0010) rides the same pipeline as its metric (ADR-0011):
    // every sentence that finished reports its spoken audio and its paced gap separately.
    const spoken = tts.filter((s) => s.ok && s.meta.cancelled === false);
    expect(spoken.length).toBeGreaterThan(0);
    for (const s of spoken) {
      expect(s.meta).toMatchObject({ audioMs: 120, pace: PACE_DEFAULT });
      expect(s.meta.gapMs).toBeGreaterThanOrEqual(gapMsFor('sentence', PACE_DEFAULT));
    }
