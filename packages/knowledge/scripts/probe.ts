...
const topic = process.argv.slice(2).join(' ').trim() || 'swift fundamentals';
const apiKey = process.env.OPENAI_API_KEY_FREE?.trim();
const modelName = process.env.PEN_LLM_OUTLINE_MODEL?.trim() || process.env.PEN_LLM_MODEL?.trim() || 'gpt-5-mini';
const baseURL = process.env.PEN_LLM_BASE_URL?.trim();

const model: LanguageModel = apiKey
  ? new OpenAILanguageModel({ apiKey, model: modelName, reasoningEffort: 'low', ...(baseURL ? { baseURL } : {}) })
...
const observer: KnowledgeObserver = {
  event: (name, data) => {
    if (name === 'knowledge.page' || name === 'knowledge.page_skipped' || name === 'knowledge.reference') log(`  [${stamp()}] ${name.replace('knowledge.', '')} ${String(data.url)}${data.reason ? ` (${String(data.reason)})` : ''}`);
    else log(`  [${stamp()}] ${name} ${JSON.stringify(data)}`);
  },
  error: (area, error, data) => log(`  [${stamp()}] ERROR ${area}: ${error instanceof Error ? error.message : String(error)} ${data ? JSON.stringify(data) : ''}`),
};

async function main(): Promise<void> {
  log(`Topic: "${topic}" · model: ${apiKey ? modelName : 'none (heuristic outline; set OPENAI_API_KEY_FREE in .env)'} · search: none (seeds only)`);
  const onten = createOnten();
  const resolution = await onten.registry.resolveTopic({ text: topic, language: 'en', locale: 'en-US', band: 'beginner' });
  log(`Resolved: ${resolution.title} (${resolution.canonicalKnowledgeId}, ${resolution.domainBoundary}, ${resolution.match})`);

  const controller = new AbortController();
  process.on('SIGINT', () => controller.abort());
  const builder = new CorpusBuilder({ compiler: onten.compiler, model, policy: onten.policy, search: new NoSearch(), observer });

  const prepared = await builder.prepare({
    resolution,
    signal: controller.signal,
    onProgress: (p) => log(`[${stamp()}] ${p.stage.padEnd(11)} ${String(Math.round(p.fraction * 100)).padStart(3)}%  ${p.status}  (${p.sourcesFetched}/${p.sourcesFound})`),
  });
  ...
  log(`Units: ${pack?.units.length ?? 0} · sources: ${pack?.sources.length ?? 0} · references (cite-only): ${prepared.references.length}`);
  log(`Evaluation: ${pack?.evaluation.development.length ?? 0} development + ${pack?.evaluation.negative.length ?? 0} negative`);
