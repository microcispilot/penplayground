...
export function fakeFetch(routes: Record<string, Route | string>, log: FetchLogEntry[] = []): typeof fetch {
  return async (input, init) => {
    ...
    return new Response(r.body, { status: r.status ?? 200, headers: { 'content-type': r.type ?? guessType(url) } });
  };
}

/** Enough markdown for several Onten units. */
export function markdownDoc(title: string, sections = 4): string {
  const parts = [`# ${title}`, '', `${title} is explained here in enough detail for a lesson segment to be built from it.`];
  for (let i = 1; i <= sections; i++) {
    parts.push('', `## ${title} part ${i}`, '', `Paragraph ${i} of ${title}. `.repeat(12).trim(), '', '```swift', `let value${i} = ${i}`, '```');
  }
  return parts.join('\n');
}

export function htmlDoc(title: string, paragraphs = 6): string {
  const body = Array.from({ length: paragraphs }, (_, i) => `<p>Paragraph ${i + 1} about ${title}. ${'More detail here. '.repeat(8)}</p>`).join('\n');
...
export function fakeCompilation(opts: { interactiveAfter: number; packId?: string } = { interactiveAfter: 2 }): FakeCompilation {
  ...
  const receipt = (): ProvisionalReceipt => ({ status: 'partial', evidenceTier: 'unverified_live_source', attribution: '', mayAuthorizeConsequentialDecision: false, packId, unitCount: sources.length * 3, cost: 0 });
  const emit = (phase: CompileProgress['phase']) => {
    for (const l of listeners) l({ sourcesReceived: sources.length, unitsCompiled: sources.length * 3, phase });
  };
  const compilation: ProgressiveCompilation = {
    ...
    finishSources: (ev) => {
      ...
      const ok = sources.length > 0 && (ev?.development.length ?? 0) > 0 && (ev?.negative.length ?? 0) > 0;
      emit(ok ? 'qualified' : 'failed');
      resolveBackground(ok ? { packId, packRevision: '2', digest: 'digest', unitCount: sources.length * 3 } : null);
    },
    onProgress: (listener) => {
...
export const OUTLINE_VALUE = {
  ...
  queries: ['swift constants and variables tutorial', 'swift optionals explained', 'swift optional binding examples', 'swift control flow guide', 'swift functions tutorial', 'swift closures explained', 'swift common beginner mistakes', 'swift let vs var'],
  candidateUrls: [] as string[],
};

export const EVALSET_VALUE = {
  development: Array.from({ length: 6 }, (_, i) => ({ question: `Development question number ${i + 1} about Swift?` })),
  negative: Array.from({ length: 4 }, (_, i) => ({ question: `Negative question number ${i + 1} about something else?` })),
};

export function fakeModel(overrides: { candidateUrls?: string[]; failOutline?: boolean; failEvalset?: boolean } = {}): FakeLanguageModel {
  const completions: Array<{ purpose: string; value: unknown }> = [];
  if (!overrides.failOutline) completions.push({ purpose: OUTLINE_PURPOSE, value: { ...OUTLINE_VALUE, candidateUrls: overrides.candidateUrls ?? [] } });
...
export function resolutionFor(title: string, ckid = `en.${title.toLowerCase().replace(/\s+/g, '-')}`): TopicResolution {
  return { canonicalKnowledgeId: ckid, title, domainBoundary: 'computing-data', match: 'miss', packId: null, lessonMemoId: null, score: 0 };
}

export function seedFor(id: string, pattern: RegExp, label: string, urls: Array<{ url: string; title: string; transform?: Transform; api?: boolean }>): Seed {
  return {
    id,
    pattern,
    label,
    priority: 0,
    targets: () => urls.map((u) => ({ url: u.url, title: u.title, transform: u.transform ?? 'none', ...(u.api ? { api: true } : {}) })),
  };
}
...
export const TEST_BUDGET = { perHostGapMs: 0, concurrency: 3, timeoutMs: 2_000, backgroundMs: 10_000 };

export const POLICY = PEN_HOST_POLICY;

export function collectProgress(): { list: PreparationProgress[]; onProgress: (p: PreparationProgress) => void } {
