...
function guideSeed(count: number, delays: number[] = [], transform: 'none' | 'docc' = 'none') {
  ...
  return {
    seed: seedFor('guide', /example guide/, 'the example guide', urls),
    routes,
...
describe('CorpusBuilder', () => {
  it('(a) emits documents to the compilation in arrival order, one at a time', async () => {
    const hosts = [
      'https://a.example.org/a.md',
      'https://b.example.org/b.md',
      'https://c.example.org/c.md',
    ];
    const routes = {
    ...
    const seed = seedFor(
      ...
      /example guide/,
      'three hosts',
      hosts.map((url, i) => ({ url, title: `Doc ${i}` })),
    );
    const fake = fakeCompilation({ interactiveAfter: 3 });
    const builder = new CorpusBuilder({
      compiler: { startProgressiveCompilation: () => fake.compilation },
      model: fakeModel(),
      policy: POLICY,
      search: new NoSearch(),
    ...
    const prepared = await builder.prepare({
      resolution: resolutionFor('Example guide'),
    ...
    expect(
      fake.sources.every((s) => s.mediaType === 'text/markdown' && s.text.includes('```swift')),
    ).toBe(true);
  });
  ...
  it('(b) resolves on the interactive promise before every source is fetched, then finishes in the background with an evaluation set', async () => {
    ...
    const builder = new CorpusBuilder({
      compiler: { startProgressiveCompilation: () => fake.compilation },
      model: fakeModel({ candidateUrls: [urls[0] ?? ''] }),
      policy: POLICY,
      search: new NoSearch(),
    ...
    // The outline repeated a seed URL: deduped, fetched exactly once.
    expect(log.filter((l) => l.url === urls[0])).toHaveLength(1);
  });
  ...
  it('(c) never ingests developer.apple.com: it is excerpted (≤ 300 chars) into references, from the search snippet when there is one', async () => {
      ...
      search: async () => [
        {
          ...
          snippet: 'An ordered, random-access collection. '.repeat(20),
          score: 0.9,
        },
      ],
    };
    ...
    const builder = new CorpusBuilder({
      compiler: { startProgressiveCompilation: () => fake.compilation },
      model: fakeModel({ candidateUrls: [apple] }),
      policy: POLICY,
      search,
      fetchImpl: fakeFetch(allRoutes, log),
      seeds: [seed],
      budget: TEST_BUDGET,
    });
    const prepared = await builder.prepare({
  ...
  it('(d) honours robots.txt disallow rules', async () => {
    ...
    expect(observer.events.find((e) => e.name === 'knowledge.robots_blocked')?.data.url).toBe(
  ...
  it('(f) reports progress with monotonic fraction and stage, honest status lines, and ends qualified at 1', async () => {
    ...
    expect(
      list.some((p) => p.stage === 'fetching' && p.status.startsWith('Reading the example guide')),
    ).toBe(true);
  ...
  it('fails honestly when nothing readable was found and reports the error through the observer', async () => {
    const seed = seedFor('empty', /example guide/, 'an empty site', [
      { url: 'https://empty.example.org/a.md', title: 'A' },
    ]);
    const routes = {
      'https://empty.example.org/a.md': { body: 'too short', type: 'text/markdown' },
    };
    const fake = fakeCompilation({ interactiveAfter: 1 });
    const observer = recordingObserver();
    const { list, onProgress } = collectProgress();
    ...
    ).rejects.toThrow(/no usable source/);
    expect(list.at(-1)?.stage).toBe('failed');
    expect(observer.errors.some((e) => e.area === 'knowledge.prepare')).toBe(true);
    expect(
      observer.events.some((e) => e.name === 'knowledge.page_skipped' && e.data.reason === 'empty'),
    ).toBe(true);
  });
  ...
  it('falls back to a heuristic outline and evaluation when the model fails, and still qualifies', async () => {
    ...
    const builder = new CorpusBuilder({
      compiler: { startProgressiveCompilation: () => fake.compilation },
      model: fakeModel({ failOutline: true, failEvalset: true }),
      policy: POLICY,
      search: new NoSearch(),
      fetchImpl: fakeFetch(routes),
      seeds: [seed],
    ...
    expect(fake.evaluation()?.development).toHaveLength(6);
    expect(observer.errors.map((e) => e.area).sort()).toEqual([
      'knowledge.evalset',
      'knowledge.outline',
    ]);
  });
  ...
  it('converts HTML search hits to markdown, skips non-text and rejects private hosts', async () => {
    ...
    const allRoutes = {
      ...routes,
      [html]: htmlDoc('Post'),
      [binary]: { body: 'xxx', type: 'image/png' },
    };
    const fake = fakeCompilation({ interactiveAfter: 2 });
    const log: Array<{ url: string; at: number }> = [];
    const observer = recordingObserver();
    ...
    expect(
      observer.events.some((e) => e.name === 'knowledge.page_skipped' && e.data.reason === 'type'),
    ).toBe(true);
  });
  ...
  it('works end to end against the real Onten MockCompiler: provisional pack, then a qualified pack with evaluation', async () => {
    ...
    const resolution = await onten.registry.resolveTopic({
      text: 'I want to learn the example guide',
      language: 'en',
      locale: 'en-US',
      band: 'beginner',
    });
