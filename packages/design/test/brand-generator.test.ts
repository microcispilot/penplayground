...
describe('the brand red keeps its red, and keeps everything else grey', () => {
   ...
   * land there, and the brand does not — it is the owner's #A9124A in both
  ...
  it('is one hex in both themes, because the dark scheme never redeclares it', () => {
    expect(brand('light')).toBe('#a9124a');
  ...
  /**
   * And by night the text role is the palette's rose (ADR-0054, amended):
   * the owner found the brand itself not visible as an outlined button's
   * label and border and asked for a tint. 6.6:1 on `surface`; the highest
   * grey, at 4.3:1, is the one step it misses and is recorded below.
   */
  it('dark: brand-coloured text is rose, and reads on the surfaces it sits on', () => {
    expect(primary('dark')).toBe('#e1799d');
    for (const role of ['surface', 'surface-container', 'surface-container-high']) {
      const ground = declared(themeBlock('dark'), role) ?? '';
      expect(
        contrast(rgb(primary('dark')), rgb(ground)),
        `dark primary on --color-${role} ${ground}`,
      ).toBeGreaterThan(4.5);
    }
    const highest = rgb(declared(themeBlock('dark'), 'surface-container-highest') ?? '');
    expect(contrast(rgb(primary('dark')), highest)).toBeCloseTo(4.34, 1);
  });
