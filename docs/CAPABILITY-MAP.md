...
| `onten` | Onten host adapter, and nothing else. Mock today (registry, progressive compile, AnswerContext assembly, Canonical Question Memo) behind the exact `ContextClient` surface; real binary/service later. Two abilities: give it documents (`learn`), get an AnswerContext for any question inside 20 ms (`query`). Depends on `contracts` alone so it stays deletable in one commit. See ADR-0019 and `docs/ONTEN-BOUNDARY.md`. | contracts | `packages/onten` |
...
| `board` | The shared board: tldraw renderer, ink-text (handwriting) shape, code/markdown shape, stroke animator, camera director, cue → shape executor, PNG snapshot export. | contracts, design | `packages/board` |
| `session-engine` | Server-side classroom brain: session state machine (preparing/live/paused/listening/answering/checking/ended), lesson planner, turn loop (perceive → decide → act → verify), cue sequencer, host/guest authority, transcript, recording ledger, and the **lesson memo** (lessons we already generated, reused by the next learner — Pen's own cache, never Onten's). | contracts, onten, llm, voice(server side of TTS), knowledge | `packages/session-engine` |
