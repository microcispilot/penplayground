...
5. **Measurement.** Every step is a product event (`ad_requested … ad_clicked`) through the app's
   ...
   revenue line (`PEN_AD_ECPM_USD / 1000` per completed ad) as a negative cost under `ads`, so
   the per-session economics show ads offsetting cost. `ad_event` folds into the generic
   telemetry `report` message when that lands.
