...
- **Session panel** — the room's right-hand column (ADR-0019): everyone on the call, the conversation, and the composer. Folds away from a chevron on its own edge; below 1024 px it comes over the board as a drawer.
- **Interaction event** — what a participant did (typed / spoke a question, interrupted, paused, skipped an ad, reacted, toggled captions or the mic…) or was shown (screen, phase, note, check, ad, recap, first audio, answer start, board op done), reported by the client and stamped by the server. A ledger entry (`interaction`).
