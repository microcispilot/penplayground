...
- [x] **A taste for a visitor, a shelf for an account, answers for a plan (ADR-0040).** Visitor: intake and prepared lessons without limit, Elena Ruiz or Soren Vale at random per visit, Jev for check-ins and intent, questions heard and answered with the upgrade line plus a calm card to Pricing, nothing to keep. Free account: plus history, your sessions, saved, liked, own recording, and one custom session (`PEN_FREE_CUSTOM_SESSIONS`, `participants.custom_sessions`), then "upgrade" with the ready lessons. Paid unchanged, plus a default expert of their own in the search box. `FeatureRule.anonymous`, four new flags, no daily count, `PEN_FEATURE_OVERLAY` for the Playwright servers, one sign-in sheet with two header doors. Open: the console's visitor column; an end-to-end test of the upgrade card.
