/*
 * Google Identity Services, loaded on demand — a copy of
 * `packages/app/src/lib/google.ts`.
 *
 * Deliberately a copy: the console must not depend on `@pen/app`, which would
 * pull the board, the audio clock and tldraw into a settings screen. This is a
 * thin adapter over Google's own SDK and changes only when Google's does.
 */
/**
