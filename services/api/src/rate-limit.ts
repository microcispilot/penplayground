...
/**
 * The key an unauthenticated caller is limited by. `X-Real-IP` is set by our
 * own edge (`deploy/nginx/pen-playground.conf.example`) from the peer address,
 * so it is the trustworthy one; nginx *appends* to `X-Forwarded-For`, whose
 * first hop is whatever the client sent, so it is only a fallback and only
 * ever its first entry — the whole header would let one caller mint unlimited
 * distinct buckets and slip past the limit.
 */
export function clientKey(headers: { header(name: string): string | undefined }): string {
  ...
  return forwarded ? forwarded.slice(0, 64) : 'local';
}
